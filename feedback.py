...
class FeedBack:
    ...
    def __init__(self,master):
        ...
        self.style.configure('TFrame',background='#D6A2E8')
        self.style.configure('TLabel',background = '#D6A2E8',font = ('Arial',11))
        self.style.configure('TButton',background = '#D6A2E8',font = ('Arial',11))
        self.style.configure('Header.TLabel',font=('Arial',18,'bold'))

        self.frame_header = ttk.Frame(master)
        self.frame_header.pack()
        self.logo = PhotoImage(file='res.gif')

        ttk.Label(self.frame_header,image = self.logo).grid(row=0,column=0,rowspan=2)
        ttk.Label(self.frame_header,text = 'Help Me To Make Improve',style = 'Header.TLabel').grid(row=0,column=1)
        ttk.Label(self.frame_header,wraplength=320,text = "I am glad you are playing this game."
                                                "Thank you for playing, please help me to improve by giving feedback").grid(row=1,column=1)
